 ...
 * @link http://www.hufersil.com.br
 */
class Lumine_Dialect_MySQLi extends Lumine_EventListener implements Lumine_Dialect_IDialect
{
		...
		try
		{
			...
			switch($mode)
			{
				case Lumine_Base::FETCH_ROW:
					$native_mode = MYSQLI_NUM;
				break;
				
				case Lumine_Base::FETCH_BOTH:
					$native_mode = MYSQLI_BOTH;
				break;
				
				case Lumine_Base::FETCH_ASSOC:
				default:
					$native_mode = MYSQLI_ASSOC;
			}
	...
	public function fetch_row($rowNumber)
	{
	    ...
	    $rs = $this->resultList[$this->getObjectId()];
	    $rs->data_seek($rowNumber);
